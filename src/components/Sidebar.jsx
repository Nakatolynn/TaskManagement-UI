...
export default function Sidebar() {
  const items = [
    ...
    { key: 'docs', label: 'Docs', icon: <ArticleIcon /> },
    { key: 'dashboards', label: 'Dashboards', icon: <DashboardIcon /> },
    { key: 'apps', label: 'Apps', icon: <WidgetsIcon /> },
  ]

  return (
    <Box sx={{ width: 260, bgcolor: 'background.paper', height: '100vh', borderRight: '1px solid rgba(255,255,255,0.04)' }}>
      ...
      <List>
        {items.map(i => (
          <ListItem key={i.key} disablePadding>
            <ListItemButton>
              <ListItemIcon sx={{ color: 'text.secondary' }}>{i.icon}</ListItemIcon>
